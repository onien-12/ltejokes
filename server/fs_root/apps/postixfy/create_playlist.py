...
def extract_metadata_from_mp3(file_path):
    ...
    try:
        ...
        print(f"    Warning: Could not get duration from mediainfo for {file_path.name}: {e}")
    ...
    try:
        ...
        if audiofile and audiofile.tag: # Check if tags exist
            if audiofile.tag.title:
                metadata['title'] = file_name.split("-")[1].strip().replace(".mp3", "") if len(file_name.split("-")) == 2  else audiofile.tag.title
                print(f"    Title: {metadata['title']}")
            if audiofile.tag.artist:
                metadata['author'] = file_name.split("-")[0].strip().replace(".mp3", "") if len(file_name.split("-")) == 2  else audiofile.tag.title
            ...
            if audiofile.tag.images: # tag.images is a list of ImageFrame objects
                ...
                picture = next((img for img in audiofile.tag.images if img.picture_type == eyed3.id3.frames.ImageFrame.FRONT_COVER), None)
                ...
                if picture:
                    ...
                    metadata['image_format'] = picture.mime_type # e.g., 'image/jpeg'
...
def process_mp3_directory(directory_path):
    ...
    mp3_files = sorted(list(root_path.glob("*.mp3"))) # Get all MP3s, sorted for consistency
    ...
    for mp3_file_path in mp3_files:
        ...
        if extracted_data['image_data']:
            image_extension = extracted_data['image_format'].split('/')[-1] if extracted_data['image_format'] else "jpg"
            ...
            import hashlib
            image_hash = hashlib.md5(extracted_data['image_data']).hexdigest()[:8]
            image_title_for_filename = sanitize_filename(song_metadata['title'] or mp3_file_path.stem)
            ...
            try:
                ...
                song_metadata['imagePath'] = str(Path("images") / image_file_name) # Stores as "images/filename.jpg"
                print(f"    Artwork saved to: {output_image_path.relative_to(root_path)}")
            except Exception as e:
