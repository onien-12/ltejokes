...
Как и для OFDMA, значения $$ N_{cp} $$ также зависят от типа CP:
...
Для начала генерируется начальный параметр $$ c $$: $$ c_{init} = n_{RNTI} \cdot 2^{14} + q \cdot 2^{13} + \lfloor \frac{n_s}{2} \rfloor \cdot 2^9 + N^{cell}_{ID} $$, где $$ n_{RNTI} $$ - RNTI, присвоенный текущей PDSCH передаче, $$ N^{cell}_{ID} $$ - Cell Identity (физический ID вышки), $$ n_s $$ - номер текущего слота. О :glossary[RNTI] и Cell ID будем говорить позже, сейчас просто стоит принять, что они здесь используются.
...
1.  Scrambling. Для scrambling используется стандартная формула получения b*hat, однако отличается $$ c_{init} $$. В случае PBCH, $$ c_{init} = N^{cell}_{ID}`, где `N^{cell}_{ID} $$ - физический ID вышки.
...
1.  Scrambling - Используется стандартная формула получения b*hat, однако $$ c_{init} $$ отличается. Он определяется так: $$ c_{init} = (\lfloor n_s/2 \rfloor + 1) \cdot (2N^{cell}_{ID} + 1) \cdot 2^9 + N^{cell}_{ID} $$: Кол-во бит здесь будет равно 31.
...
:::margin{ml=56}
  ...
  4.2.4. Затем производится операция изменения порядка столбцов в соотвествии со следующим паттерном: `1, 17, 9, 25, 5, 21, 13, 29, 3, 19, 11, 27, 7, 23, 15, 31, 0, 16, 8, 24, 4, 20, 12, 28, 2, 18, 10, 26, 6, 22, 14, 30` (таблица 36.212 5.1.4-2). Это значит, что в 0-м столбце будет находится 1-й столбец, в 1-м - 17-й, во 2-м - 9-й, и т.д.
...
При этом тут есть Pseudo-random Gold Sequence $$ c^{(q)}(i) $$ - для неё есть свой $$ c_{init} $$. Он определяется так:
...
Для форматов 1a или 1b массив бит $$ b(0),\dots,b(M_{bit} - 1) $$ модулируется согласно таблице 36.211 5.4.1-1:
...
:::
2.  $$ w_{n^{(\tilde{p})}_{oc}}(m) $$ - ортогональный код, который зависит от $$ N^{PUCCH}_{SF} $$ и $$ n^{(\tilde{p})}_{oc}(n_s) $$. Значение ортогональных кодов даны в таблицах 36.211 5.4.1-2 и 5.4.1-3:
...
В LTE ресуры PUCCH располагаются по краям спектра (это мы рассмотрим далее). При этом существует несколько форматов, но только два из них нас сейчас интересуют: Format 2 и Format 1. 

Чтобы не тратить целый PRB под один формат, когда пользлвателей мало, существует Mixed PRB. В нем часть поднесущих/сдвигов отдана под Format 2, а другая часть - под Format 1.
...
1.1.4. Чтобы лучше понять, что это значит, обратимся опять к исходной формуле генерации $$ y $$. В ней используется функция $$ r $$, которая выдаёт каждый раз новый ортогональный код. Cyclic shift - это когда мы берем этот набор чисел и делаем так, что элемент под номером $$ n $$ становится элементом под номером $$ (n + m) \mod N^{PUCCH}_{seq} $$, где $$ m $$ - как раз смещение. Так вот получается, что общее количество значений $$ m $$ - это $$ N^{(1)}_{cs} $$, шаг между ними - это $$ \Delta^{PUCCH}_{shift} $$, а $$ c $$ - общее количество возможных ортогональных кодов в одном RB. Отсюда и выходит, что значение $$ c \cdot N^{(1)}{cs}/\Delta^{PUCCH}_{shift} $$ - это общее кол-во в целом доступных cyclic shift'ов для текущих выделенных ресурсов PUCCH для одного RB.

1.1.5. И получается, что так как нам досутпно всего $$ c \cdot N^{(1)}{cs}/\Delta^{PUCCH}_{shift} $$ cyclic shift'ов, то в один RB можно записать всего какое-то определённое значение различных данных. Поэтому если номер RB $$ n^{(1,\tilde{p})}_{PUCCH} $$ меньше чем общее кол-во cyclic shift'ов, мы можем назначать в этот RB данные напрямую, а если больше (когда, например, UE больше, чем ортогональных ресурсов), то надо идти обратно и назначать данные в предыдущие RB - за это отвечает $$ (n^{(1,\tilde{p})}_{PUCCH} - c \cdot N^{(1)}_{cs}/\Delta^{PUCCH}_{shift}) \mod (c \cdot N^{RB}_{sc}/\Delta^{PUCCH}_{shift}) $$. 

1.1.6. Другими словами, вывод такой: если условие выполняется (выделенный ресурс попадает в Mixed PRB), то мы используем математику для этого самого Mixed PRB, где емкость ограничена. Если условие не выполняется, то пользователь не влез в Mixed PRB (или их не хватило) и тогда он переносится в следующие, чистые PRB, которые целиком отданы под Format 1.
...
:::
:::margin{ml=56}
  2.1.1. Если мы ещё не вышли за рамки ортогональных ресурсов, то оно будет равно значению $$ N^{(1)}_{cs} $$, т.е кол-ву cyclic shift'ов в целом. Другими словами, если мы в смешанном PRB, то под Format 1 доступно только $$ N^{(1)}_{cs} $$ ресурсов.

2.1.2. Если вышли, то $$ N^{RB}_{sc} $$, т.е кол-ву subcarrier для RB (обычно 12). Другими словами, если мы в чистом PRB, у нас доступны все поднесущие блока.
...
Это выражение определяет индекс ортогонального кода $$ w_{n^{(\tilde{p})}_{oc}}(m) $$ из тех двух таблиц, которые я приводил ранее: 5.4.1-2, 5.4.1-3
::: 
...
Набор бит $$ b(0),\dots,b(19) $$ сначала проходит этап Scrambling согласно известной нам формуле: $$ \tilde{b}(i) = (b(i) + c(i)) \mod 2 $$, где $$ c $$ - псевдорандомная последовательность чисел, для которой $$ c_{init} $$ определяется следующим образом:
...
- $$ n_{oc} $$ - индекс в таблице 36.211 5.4.2C-2:
:::

:::margin{ml=56}

**Table 5.4.2C-2: Orthogonal sequences $$w_{n_{oc}}(i)$$**
...
Для псевдорандомной последовательности $$ c $$ (которую мы рассматривали ранее), $$ c_{init} $$ генерируется следующим образом:
...
:::margin{ml=14}
При этом $$ N_{cp} = 1 $$ для normal CP и $$ N_{cp} = 0 $$ для extended CP.
:::
...
$$ c_{init} $$ устроен таким образом, чтобы дать устройству возможность декодировать (или сверить) CRS. Он использует только те параметры, которые доступны UE на момент считывания CRS: ID вышки и тип CP.
...
::heading[1.1.7.1.2.1. Sequence Generation]{h=7}

Формула генерации сигнала UE-RS зависит от используемой антенны. Для $$ p = 5 $$ формула будет следующая:

$$
r_{n_s}(m) = \frac{1}{\sqrt{2}}(1 - 2 \cdot c(2m)) + j\frac{1}{\sqrt{2}}(1 - 2 \cdot c(2m + 1)) \\
\\ \\
m = 0,\dots,12N^{PDSCH}_{RB} - 1
$$
:::margin{ml=28}
 Где
  - $$ N^{PDSCH}_{RB} $$ - кол-во RB в текущем канале PDSCH
  
При этом $$ c_{init} $$ определяется как:
...
А для антенн $$ p \in \{7, 8, \dots, 14\} $$ генерация UE-RS будет производится в соответсвии с:
$$
r_{n_s}(m) = \frac{1}{\sqrt{2}}(1 - 2 \cdot c(2m)) + j\frac{1}{\sqrt{2}}(1 - 2 \cdot c(2m + 1)) \\
\\ \\
m = \begin{cases}
 0,\dots,12N^{max,DL}_{RB} - 1 & \text{normal CP} \\
 0,\dots,16N^{max,DL}_{RB} - 1 & \text{extended CP} 
\end{cases}
$$
:::margin{ml=28}
 При этом $$ c_{init} $$ определяется как:
...
$$
c_{init} = (\lfloor n_s / 2 \rfloor + 1) \cdot (2n^{(n_{SCID})}_{ID} + 1) \cdot 2^{16} + n_{SCID}
$$

 Где 
 - $$ n_{SCID} $$ идёт из DCI форматов 2B, 2C, 2D, 6-1A, 7-1E, 7-1F, 7-1G. В случае использования других форматов, $$ n_{SCID} = 0 $$. 
 - $$ n^{(i)}_{ID} $$ равен $$ N^{cell}_{ID} $$ если в DCI не определён параметр $$ n^{DMRS,i}_{ID} $$. Иначе, $$ n^{(i)}_{ID} = n^{DMRS,i}_{ID} $$
:::

::heading[1.1.7.1.2.2. Mapping to resource elements]{h=7}
