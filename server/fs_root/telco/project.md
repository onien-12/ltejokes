...
:::margin{ml=28} 
  ::fs-image{path="/telco/other/images/PSK.png" name="PSK" maxWidth=600px}
...
::fs-image{path="/telco/other/images/16QAM.png" name="16QAM Constellation" maxWidth=600px}
...
::fs-image{path="/telco/other/images/MIMOModel.png" name="MIMO Model" maxWidth=600px}
...
::fs-image{path="/telco/other/images/MIMOChannel.png" name="MIMO Channel Information Matrix" maxWidth=400px}
...
В общем тут уже должно быть ясно, что стоит искать метод, который бы как-нибудь не использовал Channel Information Matrix ($$ H $$). И такой метод есть:
 ...
 В общем, я не буду вдаваться в объяснение самого SVD с точки зрения математики, как нибудь потом уже отдельно об этом напишу, там очень интересно. Сейчас что важно понять: SVD разбивает матрицу "H" на три матрицы: $$ U $$, $$ S $$ и $$ V^H $$. Идея в том, чтобы представить матрицу канала в виде какой-нибудь простой диагональной матрицы, в которой получить коэфициенты будет легко. Опять же, давайте взглянем на матрицу канала: $$ H = U*S*V^H $$. Именно $$ S $$ и является такой матрицей, но как убрать остальные множители? Тут мы можем сделать такую схему: 
  ...
  ::fs-image{path="/telco/other/images/MIMOPropagation.png" name="MIMO SVD Model" maxWidth=500px}
...
```
  ...
  ::fs-image{path="/telco/other/images/MIMOPrecodingTDConstellation.png" name="MIMO Precoding Transmit Diversity Constellation" maxWidth=500px}
...
| 4 | $$\large \frac{1}{2}\begin{bmatrix}1 & 1 & 1 & 1 \\ 1 & e^{-j 2\pi / 4} & e^{-j 4\pi / 4} & e^{-j 6\pi / 4} \\ 1 & e^{-j 4\pi / 4} & e^{-j 8\pi / 4} & e^{-j 12\pi / 4} \\ 1 & e^{-j 6\pi / 4} & e^{-j 12\pi / 4} & e^{-j 18\pi / 4} \end{bmatrix}$$ | $$\large \begin{bmatrix}1 & 0 & 0 & 0 \\ 0 & e^{-j 2\pi / 4} & 0 & 0 \\ 0 & 0 & e^{-j 4\pi / 4} & 0 \\ 0 & 0 & 0 & e^{-j 6\pi / 4} \end{bmatrix}$$ |
 ...
 При этом $$ W(i) $$ выбирается из общей таблицы 6.3.4.2.3-1, при этом в случае с двумя антеннами используется матрица номер 0. Здесь полная процедура, где собрана вся информация о всех матрицах:
  ...
  ::fs-image{path="/telco/other/images/MIMOLargeCDD.png" name="MIMO Precoding Large CDD Full Procedure" maxWidth=700px}
...
```
 ...
 Если нарисовать график, то можно увидеть следующее:
  ...
  ::fs-image{path="/telco/other/images/MIMOLargeCDDConstellation.png" name="MIMO Precoding Large CDD Constellation" maxWidth=1200px}
...
$$
 ...
 Всё меняется при использовании 4 антенн. В этом случае используется другая таблица: 6.3.4.2.3-2. В ней напрямую не указаны матрицы для использования, нужно пройти через несколько этапов прежде чем можно будет их получить. Для начала нужно знать как генерируется $$ W_n $$. Это делается по формуле, представленной в 6.3.4.2.3. Выбранное значение u0 посдатвляется в формулу $$ W_n $$ и вычисляется $$ W_n $$. Затем в определённом порядке выбираются столбцы, указанные в таблице 6.3.4.2.3-2 и соотвественно определяется сама матрица W. Я сделал полную иллюстрацию здесь:
  ...
  ::fs-image{path="/telco/other/images/MIMOPrecoding4Antenna.png" name="MIMO Precoding 4 Antenna Case" maxWidth=800px}
...
Scrambling - процесс добавления к сырым битам данных известного UE значения. Рассмотрим математику этого этапа: 
 ...
 ::fs-image{path="/telco/other/images/PHYScrambling.png" name="Scrambling" maxWidth=800px}
...
Если отрисовать как $$ c $$, $$ b $$ и $$ \tilde{b} $$ относятся друг к другу, то мы увидим следующую картину:
 ...
 ::fs-image{path="/telco/other/images/PHYScramblingDiagram.png" name="Scrambling Bit Diagram" maxWidth=1200px}
...
И также сделал такую иллюстрацию, где собрал вместе все таблицы. К ней можно обращаться чтобы удобно видеть всю информацию про Layer Mapping: 
 ...
 ::fs-image{path="/telco/other/images/PHYLayerMapper.png" name="PHY Layer Mapping" maxWidth=800px}
...
Размещение каждого символа в свои соотвествующие места в :glossary[Resource Grid]. Т.е распределение символов по времени и частоте. Каждый канал занимает своё место в Resource Grid, здесь они отмечены цветами: 
 ...
 ::fs-image{path="/telco/other/images/ResourceGrid.png" name="Resource Grid" maxWidth=1200px}
...
$$
 ...
 ::fs-image{path="/telco/other/images/PHYPBCHRG.png" name="PHY PBCH Resource Grid Mapping" maxWidth=500px}
...
PCFICH - канал, предназаченный для отправки данных о структуре управляющих каналов, например PDCCH. По нему отправляется именно кол-во OFDM символов в них.
 ...
 1. Scrambling - Используется стандартная формула получения b_hat, однако $$ c_{init} $$ отличается. Он определяется так: $$ c_{init} = (\lfloor n_s/2 \rfloor + 1) \cdot (2N^{cell}_{ID} + 1) \cdot 2^9 + N^{cell}_{ID} $$: Кол-во бит здесь будет равно 31.
...
:::
 ...
 ::fs-image{path="/telco/other/images/PHYPCFICHREMapping.png" name="PHY PBCH Resource Grid Mapping" maxWidth=700px}

 И также можно посмотреть как это будет выглядеть в действии, тут зелёным обозначен как раз PCFICH:

 ::fs-image{path="/telco/other/images/PHYPCFICHREExample.png" name="PCFICH Resource Element Mapping Example" maxWidth=1000px}
...
```
 ...
 И опять, сделал иллюстрацию, где собрал все формулы воедино: 
  ...
  ::fs-image{path="/telco/other/images/PHYPDCCH.png" name="PHY PDCCH" maxWidth=1000px}
...
Теперь рассмотрим саму процедуру генерации. Можете сразу смотреть на иллюстрацию, здесь указаны все формулы и таблицы:
  ...
  ::fs-image{path="/telco/other/images/PHYPHICH.png" name="PHY PHICH" maxWidth=1000px}
...
Для начала давайте разберемся с тем, как выглядит Frame Structure для Uplink. Сначала лучше взглянуть на иллюстрацию: 
  ...
  ::fs-image{path="/telco/other/images/PHYUplinkFrame.png" name="PHY Uplink Frame Structure" maxWidth=1200px}
...
Тут я сделал иллюстрацию, где указал все формулы и матрицы, используемые для PUSCH: 
  ...
  ::fs-image{path="/telco/other/images/PHYPUSCH.png" name="PHY PUSCH" maxWidth=1000px}
...
На этой иллюстрации я обьеденил все, что было только что описано. Тут представлена полная процедура генерации символов $$ \zeta $$ для PUCCH format 1, 1a, 1b:   
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHFormat1.png" name="PHY PUCCH Format 1" maxWidth=1400px}
...
На этой иллюстрации я также собрал все формулы в месте:
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHFormat2.png" name="PHY PUCCH Format 2" maxWidth=1100px}
...
И снова, я сделал иллюстрацию, где указал все использующиеся формулы для этого формата: 
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHFormat3.png" name="PHY PUCCH Format 3" maxWidth=1100px}
...
Для этого формата я уже сделал иллюстрацию: 
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHFormat5.png" name="PHY PUCCH Format 5" maxWidth=1200px}
...
Из формулы видно, что она зависит от номера слота $$ n_s $$ и меняет своё поведение в зависимости от чётности этого параметра. В частности, если $$ n_s \mod 2 = 0 $$ (т.е четный), то весь PUCCH будет находиться внизу RG (т.е расположение по частоте будет ниже в RG). Иначе, весь PUCCH будет находится вверху RG. Это я отобразил на диаграмме:
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHREMapping.png" name="PHY PUCCH Resource Element Mapping" maxWidth=800px}
...
```
  ...
  ::fs-image{path="/telco/other/images/PHYPUCCHREMappingDefault.png" name="PHY PUCCH Resource Element Mapping Default Configuration" maxWidth=1000px}
...
Лучше понять сигнал $$ r $$ поможет иллюстрация:
  ...
  ::fs-image{path="/telco/other/images/PHYCRSConstellation.png" name="PHY CRS Constellation" maxWidth=800px}
...
При изменении ID вышки CRS будет смещаться по частоте: 
  ...
  ::fs-image{path="/telco/other/images/PHYCRSNcellIDMapping.png" name="PHY CRS RE N_cell_id Mapping" maxWidth=1000px}

А на разных антеннах CRS будет всегда занимать разные RE: 

  ::fs-image{path="/telco/other/images/PHYCRSantenna_portMapping.png" name="PHY CRS RE antenna_port Mapping" maxWidth=1000px}
