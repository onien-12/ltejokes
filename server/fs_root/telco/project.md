...
Размещение каждого символа в свои соотвествующие места в :glossary[Resouse Grid]. Т.е распределение символов по времени и частоте. Каждый канал занимает своё место в Resource Grid, здесь они отмечены цветами: 
...
:::
 ...
 где 
  ...
  - $$ \Delta^{PUCCH}_{shift} $$ идёт из параметра :glossary[deltaPUCCH-shift] в SIB2
  - $$ N^{(1)}_{cs} $$ идёт из параметра :glossary[nCS-AN] в SIB2
  - $$ N^{RB}_{sc} = 12 $$
  - $$ n^{(1,\tilde{p})}_{PUCCH} $$ - номер ресурса, который идёт из верхних уровней (а именно из DCI, совместно с SIB2 (поле :glossary[n1PUCCH-AN]) (там используется формула, которая состоит из двух параметров), но об этом в разделе про процедуры)
 :::
...
:::margin{ml=28}
Тут
 - $$ N^{(2)}_{RB} $$ - идёт из поля :glossary[PUCCH-ConfigCommon.nRB-CQI] в SIB2.
 - $$ n^{(2,\tilde{p})}_{PUCCH} $$ - идёт из поля :glossary[CQI-ReportPeriodic.setup.cqi-PUCCH-ResourceIndex] в :glossary[RRC Connection Reconfiguration].
 - $$ N^{(1)}_{cs} $$ - идёт из поля :glossary[PUCCH-ConfigCommon.nCS-AN] в :glossary[SIB2].
:::
...
Параметр :glossary[numberOfPRB-format4] описан не в последнем релизе 36.331 (откуда я беру описания SIB2). Он описан в релизе 36.331 13.5.0 и находится в поле `PUCCH-Config.PUCCH-ConfigDedicated.pucch-Format.format4.format4-resourceConfiguration.numberOfPRB-format4` в SIB2.
...
| $$1$$      | $$[+1 \quad +1 \quad +1 \quad +1 \quad +1 \quad +1 \quad -1 \quad -1 \quad -1 \quad -1 \quad -1 \quad -1]$$ |
 ...
 Для этого параметра нет формулы для генерации. Номер ортогонального кода идёт из :glossary[RRC Connection Reconfiguration], поля :glossary[PUCCH-Config.cdm-index-format5] (он тоже описан в 36.331 13.5.0, т.е не в последней ревизии). 
:::
